...
export const HairBusinessScreen: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'finance' | 'evaluators' | 'rules'>('dashboard'); // REMOVIDO: 'stored-hair' tab
  ...
  // REMOVIDO: Estados relacionados ao formulário de Cabelo Guardado
  ...
  // --- Stored Hair Handlers (REMOVIDO) ---
  // As funções de Cabelo Guardado foram removidas daqui, pois a funcionalidade será acessada diretamente do menu principal.
  // Os estados `newlyRegisteredStoredHair`, `showDeliveryConfirmModal`, `deliveryCodeInput`, `deliveryConfirmError`, `storedHairToDeliver`
  // e as funções `handleStoredHairImageChange`, `handleSaveStoredHair`, `startEditStoredHair`, `resetStoredHairForm`,
  // `openDeliveryConfirmModal`, `handleConfirmDelivery` e a tela de sucesso `if (newlyRegisteredStoredHair)`
  // foram movidos para a tela `StoredHairScreen.tsx` (que será criada em breve, se necessário, ou integrada em outro local).
  ...
  return (
    <div className="p-4 pb-20 relative">
      ...
      <div className="flex mb-6 bg-purple-50 p-1 rounded-lg overflow-x-auto">
        ...
        {/* REMOVIDO: Botão da aba 'Cabelo Guardado' */}
      </div>
      ...
      {/* REMOVIDO: Bloco da aba 'Cabelo Guardado' */}
